...
void OnStateReceived(TX_HANDLE hStateBag)
{
	...
	if (success) {
		...
		SCREEN_HEIGHT = screenBounds.Height;	// Updating global variable for screen dimensions
	...
	if (success) {
		if (presenceData != TX_USERPRESENCE_UNKNOWN) {
			USER_PRESENT = presenceData == TX_USERPRESENCE_PRESENT ? true : false;	// Updating global variable for user's presence
	...
	if (success) {
		EYES_TRACKED = gazeTracking == TX_GAZETRACKING_GAZETRACKED ? true : false;	// Updating global variable for eyes tracked
